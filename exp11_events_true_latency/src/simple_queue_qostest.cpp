...
void busy_run_for(std::chrono::microseconds duration) {
    auto start = std::chrono::high_resolution_clock::now();
    while (std::chrono::high_resolution_clock::now() - start < duration) {
        // busy loop
        asm volatile("" ::: "memory");  // 防止编译器优化掉
    }
}
    ...
    for(int i=0;i<4;i++){
      ...
      mytimer.push_back(this->create_wall_timer(
        ...
        [this,i]() {
          ...
          std::cout << "Current thread(Timer) is running on CPU: " << cpu << std::endl;
          
          busy_run_for(std::chrono::microseconds(sleep_time[i][0]));
    ...
    for(int i=0;i<4;i++){
      for(int j=0;j<3;j++){
        ...
        [this,i,j](const std_msgs::msg::String::SharedPtr msg) {
          ...
          //if(i==3)  cnt_++;
          busy_run_for(std::chrono::microseconds(sleep_time[i][j+1]));
          if(i==2 && index_array[i][j+1]==5)  busy_run_for(std::chrono::microseconds(sleep_time[i][j+1]));
          int cpu = sched_getcpu();
          std::cout << "Current thread(Sub) is running on CPU: " << cpu << std::endl;
